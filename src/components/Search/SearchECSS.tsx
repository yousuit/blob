...
class SearchECSS extends React.Component<{
	...
	constructor(props) {
		...
		this.filterOptions = [
            // @ts-ignore
			{ messageID: 'category', swiftype: 'filter_ecss_tags', data: this.props.filterData.allContentfulFilterEcssTag },
...
/*			setTimeout(() => {
	...
	private searchFilters = () => {
        ...
        filters.page['filter_start_date'] = {
            type: 'range',
		...
		for (var prop in [this.state.filters.currTypeFilters]) {
			if ([this.state.filters.currTypeFilters].hasOwnProperty(prop)) {
                ...
                // @ts-ignore
                if(Object.values(innerObj[0])[1].length >= 1) {
                    // @ts-ignore
                    from.setFullYear(Object.values(innerObj[0])[1]);
                    from.setMonth(0);
                    from.setDate(1);
                    ...
                    filters.page['filter_start_date'] = {
                        ...
                        to: new Date().getFullYear() === to.getFullYear() ? new Date().toDateString() : to.toDateString()
                    };
                }
			}
		}
    ...
    private isFilterEmpty() {
        return this.currSearchQuery === '' && this.state.filters.currTypeFilters.filter_ecss_tags.length === 0 && this.state.filters.currTypeFilters.filter_year.length === 0;
    }
	...
	private submitHandler = (event = null) => {
        ...
        if (this.noInput()) {
			...
			let config = {
				...
				per_page: this.isFilterEmpty() ? 3 : 100,
				page: this.isFilterEmpty() ? this.state.currentPage : 1
			};

			(config as any).sort_field = { page: 'filter_start_date' };
			(config as any).sort_direction = { page: 'desc' };
	...
	private submitResponseHandler = (response: SwiftypeAPIResponse) => {
		if (deepCompare(this.state.resultData, response) === false) {
	...
	private onSelectChangeHandler = (selectedOptions, listSwiftypeId) => {
        ...
        filters.currTypeFilters[listSwiftypeId] = selectedOptions ? listSwiftypeId !== 'filter_year' ? selectedOptions.map(option => option.value) : selectedOptions.value : [] ;
	...
	render() {
		...
		for (let i = 1; i <= Math.ceil(this.state.total_result_count / (this.isFilterEmpty() ? 3 : 100)); i++) {
			if (i != 1) {
				pageNumbers.push(i);
			}
		}
			...
			if (page == next) {
				return (
					loadMore && (
                        ...
                        */
                        <a id={page} ref={page} onClick={this.handlePagination} href={null} className={`text-style-body ${styles.ctaLink} ${styles.clickable}`}>
                            <FormattedMessage id={'Show more sessions'} />
                        </a>
					)
				);
			}
					...
					<form onSubmit={this.submitHandler}>
						<div className={styles.expertFilters}>
							<div className={styles.inputWrapper}>
								...
								/>
								<button id='ecss-search-submit' className={styles.submitButton} type="submit" tabIndex={0}>
							...
							<div className={styles.filters}>
								<div className={styles.typeFilterSelectLists}>
									{this.filterOptions.map((options, index) => {
										const values = [];
										const optionsSet = options.data.edges.sort((a, b) => index === 0 && a?.node?.title?.localeCompare(b?.node?.title))
										.map(node => {
											if (this.state.filters.currTypeFilters[options.swiftype] && this.state.filters.currTypeFilters[options.swiftype].indexOf(node.node.contentful_id) >= 0) {
										...
										return (
											<div key={index} className="SelectList subsubfilterstabindexcheck">
												<label tabIndex={-1} htmlFor={'_title_' + options.swiftype}>
													<FormattedMessage id={index === 0 ? 'search_filter_list_category' : 'search_filter_list_year'} />
												</label>
												<Select
													...
													options={optionsSet}
													isMulti={index === 0 && true}
                                                    // @ts-ignore
													ref={ref => (this.selectLists[options.swiftype] = ref)}
													key={options.swiftype}
                                                    hideSelectedOptions={false}
                                                    isClearable
                                                    aria-label={this.props.intl.formatMessage({id: index === 0 ? 'search_filter_list_category' : 'search_filter_list_year'})}
												/>
											</div>
										);
									})}
...
export default injectIntl(SearchECSS);